    ...
    function loadmenu() {
        var content =
            ...
            <a class="nav-link" href="taikhoan">
                <div class="sb-nav-link-icon"><i class="fas fa-users iconmenu"></i></div>
                Khách hàng
            </a>
            <a class="nav-link" href="nhacungcap">
                <div class="sb-nav-link-icon"><i class="fas fa-users iconmenu"></i></div>
                Nhà cung cấp
            </a>
...
async function checkroleEmployee() {
    var token = localStorage.getItem("token");
    var url = 'http://localhost:8080/api/user/employee/check-role-employee';
