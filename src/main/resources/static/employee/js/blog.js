async function loadBlog() {
    ...
    var url = 'http://localhost:8080/api/blog/public/findAll';
    ...
    for (i = 0; i < list.length; i++) {
        main += ` <tr>
                    ...
                    <td>${list[i].title}</td>
                    <td><span class="desblog">${list[i].description}</span></td>
                    <td class="sticky-col">
...
async function saveBlog() {
    ...
    var url = 'http://localhost:8080/api/blog/employee/create';
...
async function deleteBlog(id) {
    ...
    var url = 'http://localhost:8080/api/blog/employee/delete?id=' + id;
    const response = await fetch(url, {
        method: 'DELETE',
        headers: new Headers({
            'Authorization': 'Bearer ' + token
