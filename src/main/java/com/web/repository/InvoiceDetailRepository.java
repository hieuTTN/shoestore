...
public interface InvoiceDetailRepository extends JpaRepository<InvoiceDetail,Long> {
    ...
    @Query("select i from InvoiceDetail i where i.invoice.userAddress.user.id = ?1 and i.size.product.id = ?2")
    List<InvoiceDetail> findByUserAndProductId(Long userId, Long productid);

    @Query("select count(i.id) from InvoiceDetail i where i.size.product.id = ?1")
